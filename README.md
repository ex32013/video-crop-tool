...
1. **启动**：
   - Windows：双击 `start.bat`（或 `python start_server.py`）
   - macOS / Linux：`chmod +x start.sh && ./start.sh`（或 `python3 start_server.py`）
   - 浏览器自动打开 `http://127.0.0.1:8003/`
2. **语言**：右上角「中 / EN」按钮切换中文 / English（记住选择）。
3. **导入视频**：点「导入视频/文件夹」浏览任意目录；或**直接把视频文件拖进窗口**自动上传导入。可多导入，用下拉框切换，「取消导入」移除当前。
4. **框选裁剪区域**：拖拽框选（选区外自动变暗），四角控制点可微调；坐标按 1920×1080 原图像素显示。
5. **定时间片段**（4 按键流程）：①创建空白片段 → ②设为起始 → ③设为结束 → ④导入当前选区；**表格行可拖拽排序**，时间轴色块**双击跳转**。
6. **导出**：编码/格式/画质/并行数可调，勾选「导出后打开文件夹」完成后自动打开目录；导出有进度条 + 完成/失败 toast 通知。
7. **批量**：全选/批量删除/批量画质/**批量改名**（片段1/2/3…）；每行「⋯」菜单可复制/清空时间/置顶/删除。
8. **坐标持久化**：导出 JSON/CSV，导入恢复（导入前有预览确认）。
9. **快捷键**：「设置」里自定义（空格播放、←/→ 步进、Ctrl+Z 撤销/重做、Ctrl+Y 重做等）。
